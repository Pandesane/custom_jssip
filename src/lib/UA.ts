...
import { Message, type SendMessageOptions } from "./Message";
...
import { RTCSession, type AnswerOptions, type Originator, type RTCSessionEventMap, type TerminateOptions } from './RTCSession'
import type { Socket, WeightedSocket } from "./Socket";
...
export type SipEventListener = <T = any>(event: { event: T; request: SIPMessage.IncomingRequest; }) => void
...
export class UA extends EventEmitter {
   ...
   *
   */
  sendOptions(target: any, body: any, options: any) {
   ...
   */
  destroyTransaction(transaction: any) {
   ...
   *  new Message
   */
  newMessage(message: any, data: any) {
   ...
   *  new Options
   */
  newOptions(message: any, data: any) {
    this._applicants[message] = message;
   ...
   * new RTCSession
   */
  newRTCSession(session: any, data: any) {
   ...
   */
  _findSession({ call_id, from_tag, to_tag }: any) {
   ...
   */
  _findDialog(call_id: any, from_tag: any, to_tag: any) {
...
function onTransportConnecting(data: any) {
  // @ts-ignore
  this.emit("connecting", data);
}

// Transport connected event.
function onTransportConnect(data: any) {
  // @ts-ignore
...
function onTransportDisconnect(data: any) {
